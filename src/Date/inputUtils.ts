...
export default function useDateInput({
  ...
  const onChangeText = (date: string) => {
    ...
    if (Number.isNaN(day) || Number.isNaN(year) || Number.isNaN(month)) {
      const inputError = `Date format must be ${inputFormat}`;
      setError(inputError)
    ...
    if (isDisabled(finalDate)) {
      const inputError = "Day is not allowed";
      setError(inputError)
      onValidationError && onValidationError(inputError)
      return
    }
    if (!isWithinValidRange(finalDate)) {
      let errors =
        validStart && validEnd
          ...
          : [
              validStart
                ? `Must be later then ${formatter.format(validStart)}`
                : '',
              validEnd
                ? `Must be earlier then ${formatter.format(validEnd)}`
                : '',
            ]
